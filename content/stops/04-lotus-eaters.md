---
id: lotus-eaters
seq: 4
phase: wanderings
title: The Lotus-Eaters
book_refs: ["Od. 9.82–104"]
coords: { lat: 33.808, lng: 10.845 }
camera: { zoom: 9 }
certainty: traditional
ancient: { grc: "Λωτοφάγοι", translit: "Lōtophágoi", el: "Λωτοφάγοι" }
modern:
  primary: { name: "Djerba", country: TN }
crew: { remaining: 528, lost_here: 0 }
summary: "A gentle people, a sweet fruit, and the first enemy with no weapon at all: forgetting that you want to go home."
excerpt:
  lines:
    - { n: 94, grc: "τῶν δʼ ὅς τις λωτοῖο φάγοι μελιηδέα καρπόν," }
    - { n: 95, grc: "οὐκέτʼ ἀπαγγεῖλαι πάλιν ἤθελεν οὐδὲ νέεσθαι," }
    - { n: 96, grc: "ἀλλʼ αὐτοῦ βούλοντο μετʼ ἀνδράσι Λωτοφάγοισι" }
    - { n: 97, grc: "λωτὸν ἐρεπτόμενοι μενέμεν νόστου τε λαθέσθαι." }
    - { n: 98, grc: "τοὺς μὲν ἐγὼν ἐπὶ νῆας ἄγον κλαίοντας ἀνάγκῃ," }
    - { n: 99, grc: "νηυσὶ δʼ ἐνὶ γλαφυρῇσιν ὑπὸ ζυγὰ δῆσα ἐρύσσας." }
    - { n: 100, grc: "αὐτὰρ τοὺς ἄλλους κελόμην ἐρίηρας ἑταίρους" }
    - { n: 101, grc: "σπερχομένους νηῶν ἐπιβαινέμεν ὠκειάων," }
    - { n: 102, grc: "μή πώς τις λωτοῖο φαγὼν νόστοιο λάθηται." }
  en: "And whosoever of them ate of the honey-sweet fruit of the lotus, had no longer any wish to bring back word or to return, but there they were fain to abide among the Lotus-eaters, feeding on the lotus, and forgetful of their homeward way. These men, therefore, I brought back perforce to the ships, weeping, and dragged them beneath the benches and bound them fast in the hollow ships; and I bade the rest of my trusty comrades to embark with speed on the swift ships, lest perchance anyone should eat of the lotus and forget his homeward way."
  cite: "Od. 9.94–102 · Greek: Perseus (ed. Murray 1919) · tr. A.T. Murray 1919"
interlinear:
  - line: 94
    words:
      - { w: "τῶν", gloss: "of our men" }
      - { w: "δʼ", lemma: "δέ", gloss: "and" }
      - { w: "ὅς", gloss: "whoever" }
      - { w: "τις", gloss: "any one" }
      - { w: "λωτοῖο", lemma: "λωτός", gloss: "of the lotus" }
      - { w: "φάγοι", lemma: "ἐσθίω", gloss: "ate — optative: whoever ever so much as tasted" }
      - { w: "μελιηδέα", lemma: "μελιηδής", gloss: "honey-sweet — the epithet Homer gives to wine, and once to homecoming itself (11.100): the lotus counterfeits νόστος" }
      - { w: "καρπόν,", lemma: "καρπός", gloss: "fruit" }
  - line: 95
    words:
      - { w: "οὐκέτʼ", lemma: "οὐκέτι", gloss: "no longer" }
      - { w: "ἀπαγγεῖλαι", lemma: "ἀπαγγέλλω", gloss: "to bring back word" }
      - { w: "πάλιν", gloss: "back" }
      - { w: "ἤθελεν", lemma: "ἐθέλω", gloss: "had the will" }
      - { w: "οὐδὲ", lemma: "οὐδέ", gloss: "nor" }
      - { w: "νέεσθαι,", lemma: "νέομαι", gloss: "to journey home — the verb inside νόστος" }
  - line: 96
    words:
      - { w: "ἀλλʼ", lemma: "ἀλλά", gloss: "but" }
      - { w: "αὐτοῦ", gloss: "right there, on the spot" }
      - { w: "βούλοντο", lemma: "βούλομαι", gloss: "they chose, preferred" }
      - { w: "μετʼ", lemma: "μετά", gloss: "among" }
      - { w: "ἀνδράσι", lemma: "ἀνήρ", gloss: "the men" }
      - { w: "Λωτοφάγοισι", lemma: "Λωτοφάγοι", gloss: "the Lotus-eaters" }
  - line: 97
    words:
      - { w: "λωτὸν", lemma: "λωτός", gloss: "lotus" }
      - { w: "ἐρεπτόμενοι", lemma: "ἐρέπτομαι", gloss: "grazing on — in the Iliad the identical phrase describes idle horses at their fodder (2.776): the men are joining the livestock" }
      - { w: "μενέμεν", lemma: "μένω", gloss: "to stay (epic infinitive)" }
      - { w: "νόστου", lemma: "νόστος", gloss: "of homecoming" }
      - { w: "τε", gloss: "and" }
      - { w: "λαθέσθαι.", lemma: "λανθάνομαι", gloss: "to forget — the root of Lethe, oblivion's river; forgetting νόστος is this poem's true death" }
  - line: 98
    words:
      - { w: "τοὺς", gloss: "these men" }
      - { w: "μὲν", lemma: "μέν", gloss: "for their part" }
      - { w: "ἐγὼν", lemma: "ἐγώ", gloss: "I" }
      - { w: "ἐπὶ", lemma: "ἐπί", gloss: "to" }
      - { w: "νῆας", lemma: "ναῦς", gloss: "the ships" }
      - { w: "ἄγον", lemma: "ἄγω", gloss: "I hauled" }
      - { w: "κλαίοντας", lemma: "κλαίω", gloss: "weeping — grief aimed at the rescue, not the loss" }
      - { w: "ἀνάγκῃ,", lemma: "ἀνάγκη", gloss: "by force" }
  - line: 99
    words:
      - { w: "νηυσὶ", lemma: "ναῦς", gloss: "in the ships" }
      - { w: "δʼ", lemma: "δέ", gloss: "and" }
      - { w: "ἐνὶ", lemma: "ἐν", gloss: "inside" }
      - { w: "γλαφυρῇσιν", lemma: "γλαφυρός", gloss: "hollow — the ships' stock epithet" }
      - { w: "ὑπὸ", lemma: "ὑπό", gloss: "beneath" }
      - { w: "ζυγὰ", lemma: "ζυγόν", gloss: "the rowing-benches (literally 'yokes')" }
      - { w: "δῆσα", lemma: "δέω", gloss: "I bound (them)" }
      - { w: "ἐρύσσας.", lemma: "ἐρύω", gloss: "having dragged (them aboard)" }
  - line: 100
    words:
      - { w: "αὐτὰρ", lemma: "αὐτάρ", gloss: "then" }
      - { w: "τοὺς", gloss: "the" }
      - { w: "ἄλλους", lemma: "ἄλλος", gloss: "rest" }
      - { w: "κελόμην", lemma: "κέλομαι", gloss: "I ordered" }
      - { w: "ἐρίηρας", lemma: "ἐρίηρος", gloss: "trusty — the comrades' fixed epithet, kept even here" }
      - { w: "ἑταίρους", lemma: "ἑταῖρος", gloss: "comrades" }
  - line: 101
    words:
      - { w: "σπερχομένους", lemma: "σπέρχομαι", gloss: "at speed, in haste" }
      - { w: "νηῶν", lemma: "ναῦς", gloss: "the ships" }
      - { w: "ἐπιβαινέμεν", lemma: "ἐπιβαίνω", gloss: "to board (epic infinitive)" }
      - { w: "ὠκειάων,", lemma: "ὠκύς", gloss: "swift" }
  - line: 102
    words:
      - { w: "μή", gloss: "lest" }
      - { w: "πώς", lemma: "πως", gloss: "somehow, by any chance" }
      - { w: "τις", gloss: "anyone" }
      - { w: "λωτοῖο", lemma: "λωτός", gloss: "of the lotus" }
      - { w: "φαγὼν", lemma: "ἐσθίω", gloss: "by eating" }
      - { w: "νόστοιο", lemma: "νόστος", gloss: "his homecoming" }
      - { w: "λάθηται.", lemma: "λανθάνομαι", gloss: "should forget — the episode closes on the same line-end pair that opened the danger (97): lotus eaten, homecoming forgotten" }
etymology:
  - lemma: "λωτός"
    stages:
      - { lang: grc, form: "λωτός", gloss: "lotus — a floating name Greek gave to quite different plants", note: "clover, fodder herbs, the Egyptian water-lily, and this fruit of forgetting all shared it" }
      - { lang: la, form: "lōtus" }
      - { lang: en, form: "lotus; lotus-eater", note: "'lotus-eater' for a dreamy idler is 19th-century English, fixed in the language by Tennyson's poem" }
    note: "The name is wandering still: in Modern Greek λωτός means the persimmon."
    sources: ["LSJ s.v. λωτός", "OED s.v. lotus-eater"]
tidbits:
  - text: "What plant was it? Herodotus, placing lotus-eating peoples on this same North African coast, describes a fruit sweet as the date; Polybius describes the Libyan jujube (Ziziphus lotus), whose berries were fermented into wine; moderns have added dates, hackberries, and outright opiates. Every identification is conjecture — the fruit grows reliably only in the poem."
    source: "Hdt. 4.177–178; Polybius, in Athenaeus 14.651 — identifications conjectural"
  - text: "Djerba's claim is ancient, not a tourist-board invention: when a Roman fleet ran aground there in 253 BC, Polybius calls the island 'the island of the Lotus-eaters', and Strabo repeats the identification of Meninx — Djerba's ancient name — with Homer's lotus-land."
    source: "Polybius 1.39.2; Strabo 17.3.17"
  - text: "The Lotus-eaters 'did not plan death for our comrades' — they are the only adversaries in the wanderings who harm no one. The episode inverts the poem's great theme of hospitality: a host can destroy you simply by making you want to stay. Note who uses force here — Odysseus, dragging his weeping men to the benches and tying them down."
    source: "Od. 9.92–99"
  - text: "Tennyson's The Lotos-Eaters (1832) grows Homer's brief episode into a full choric dream — 'In the afternoon they came unto a land / In which it seemed always afternoon' — and made 'lotus-eater' the English word for blissful truancy. The poem in turn inspired Robert Duncanson's Land of the Lotus Eaters, this stop's artwork, which the painter reportedly showed to Tennyson himself in 1863."
    source: "Tennyson, The Lotos-Eaters (1832); Ketner, The Emergence of the African-American Artist (1993)"
art:
  - file: "/art/lotus-eaters/duncanson-1861.jpg"
    title: "Land of the Lotus Eaters"
    artist: "Robert S. Duncanson"
    year: "1861"
    collection: "Collection of the Swedish Royal Family"
    license: "Public domain"
    source_url: "https://commons.wikimedia.org/wiki/File:Robert_Duncanson_-_Land_of_the_Lotos_Eaters.JPG"
now_today: "Djerba is Tunisia's holiday island — flat, palm-fringed, ringed with resort beaches — with the whitewashed market town of Houmt Souk at its center and, at El Ghriba, one of the oldest synagogues in the world, still the goal of an annual pilgrimage. The lotus-eater branding survives on hotel signs."
---
Deep content (M4): excerpt, etymology, tidbits, art, now/today.
